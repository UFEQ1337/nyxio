...
> Bot dziala wewnatrz kontenera jako non-root user (uid 1000), ale entrypoint
> (`docker-entrypoint.sh`) startuje jako root, naprawia wlasciciela bind-mounta
> `./data` i dopiero potem dropuje uprawnienia. Dlatego **reczny `chown` nie jest
> potrzebny** — deploy sam sie leczy niezaleznie od wlasciciela `./data` na hoscie.
